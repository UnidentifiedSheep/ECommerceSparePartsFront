...
export async function getProductByIdGraphql(productId: number): Promise<ProductModel | null> {
  const response = await graphqlClient.query<ProductByIdResponse>({
    query: ProductByIdDocument,
    variables: { id: productId, crossesInput: { pagination: { page: 0, size: 100 }, sortBy: [] } },
    fetchPolicy: 'cache-first',
  })
  return response.data.products.byId ? mapProduct(response.data.products.byId) : null
}
