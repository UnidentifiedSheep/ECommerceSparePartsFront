  ...
  >
    <template #header>
      <div class="dialog-heading">
        <div class="min-w-0">
          <div class="dialog-kicker">Редактирование</div>
          <h2>Закупка {{ purchase ? formatDate(purchase.purchaseDatetime) : '' }}</h2>
          <p>Поставщик и склад прихода остаются прежними, остальные параметры можно обновить.</p>
        </div>
    ...
    <div class="purchase-dialog-body">
      <aside class="purchase-summary">
        <div class="summary-total">
          <span>Итого</span>
          <strong>{{ formatCurrency(editPurchaseTotal, selectedCurrency?.currencySign) }}</strong>
        </div>
      ...
      <el-form label-position="top" class="purchase-form">
        <section class="purchase-section purchase-section--main">
          ...
          <div class="form-grid">
            ...
            <el-form-item label="Логистика" class="span-3">
              <div class="flex h-8 items-center">
                <el-switch v-model="form.withLogistics" active-text="Да" inactive-text="Нет" />
              </div>
            </el-form-item>

            <el-form-item v-if="form.withLogistics" label="Склад отправки" class="span-6">
        ...
        <section class="purchase-section">
          ...
          <div v-if="form.items.length === 0" class="empty-items">
            ...
            <div class="mt-1 text-sm text-slate-500">Добавьте хотя бы один товар для сохранения закупки.</div>
          </div>
              ...
              <div class="item-product">
                ...
                <div class="min-w-0">
                  ...
                  <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                    ...
                    <span v-if="item.product.producerName">{{ item.product.producerName }}</span>
                  </div>
...
interface EditPurchaseItemForm {
  id?: number | null
  product: EditPurchaseProductForm
  count: number
  price: number
...
const completionSteps = computed(() => [
  ...
  { label: 'Дата', done: form.purchaseDate !== '' },
...
const canSave = computed(() => (
  ...
  && form.purchaseDate !== ''
  && form.items.length > 0
  && form.items.every((item) => item.product.id > 0 && item.count > 0 && item.price >= 0)
...
function resetForm() {
  ...
  form.comment = props.purchase?.comment ?? ''
  form.withLogistics = !!props.purchase?.logistics
  ...
  form.items = props.content
    ...
    .map((item) => ({
      ...
      count: item.count,
      price: item.price,
      calculateLogistics: !!item.logistics,
      comment: item.comment ?? '',
      logisticsCost: item.logistics?.price,
    }))
  clearLogisticsPreview(false)
}
...
function clearLogisticsPreview(clearCost = true) {
  logisticsCurrencyId.value = undefined
  form.items.forEach((item) => {
    if (clearCost) item.logisticsCost = undefined
...
const recalculateLogistics = useDebounceFn(async () => {
  ...
  const itemsToCalculate = form.items
    ...
    .filter(({ item }) => item.calculateLogistics && item.product.id > 0 && item.count > 0)
...
async function save() {
  if (!canSave.value || !props.purchase || !form.currencyId) return

  isSaving.value = true
  try {
    await editPurchase(props.purchase.id, {
      currencyId: form.currencyId,
      purchaseDateTime: form.purchaseDate,
    ...
    isOpen.value = false
    emit('updated', props.purchase.id)
  } finally {
...
watch(
  ...
  (state) => {
    if (state.open) recalculateLogistics()
  },
  { deep: true },
)
</script>
...
:deep(.edit-purchase-dialog .el-dialog__header) {
  padding: 26px 36px 22px;
}

:deep(.edit-purchase-dialog .el-dialog__body) {
  max-height: min(760px, calc(100vh - 178px));
  overflow-y: auto;
  padding: 0;
}

:deep(.edit-purchase-dialog .el-dialog__footer) {
