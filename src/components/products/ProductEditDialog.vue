<template>
  <el-dialog v-model="isOpen" title="Редактировать продукт" width="620">
    <el-form label-position="top">
      <div class="grid grid-cols-[minmax(0,1fr)_220px] gap-3">
        ...
        <el-form-item label="Цвет-индикатор">
          <div class="flex h-8 items-center gap-2">
            <el-color-picker v-model="form.indicator" show-alpha />
            <el-button v-if="form.indicator" size="small" text @click="form.indicator = null">Очистить</el-button>
          </div>
        </el-form-item>
      </div>
...
async function save() {
  ...
  if (sku.length < 3 || !name || !form.producerId) {
    ElNotification({
      title: 'Проверьте продукт',
      message: 'Нужны артикул от 3 символов, название и производитель.',
      type: 'warning',
    })
    return
  }
